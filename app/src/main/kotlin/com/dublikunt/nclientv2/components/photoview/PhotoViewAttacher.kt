...
import com.dublikunt.nclientv2.components.photoview.Compat.postOnAnimation
import com.dublikunt.nclientv2.components.photoview.Util.checkZoomLevels
import com.dublikunt.nclientv2.components.photoview.Util.hasDrawable
import com.dublikunt.nclientv2.components.photoview.Util.isSupportedScaleType
import kotlin.math.*
...
class PhotoViewAttacher(private val mImageView: ImageView) : OnTouchListener,
    ...
    fun setScale(
        ...
        var scale = scale
        scale = min(scale, mMaxScale)
        scale = max(scale, mMinScale)
        if (animate) {
            mImageView.post(
                AnimatedZoomRunnable(
                    this.scale, scale,
    ...
    private inner class FlingRunnable(context: Context?) : Runnable {
        ...
        fun fling(
            ...
            val startX: Int = (-rect.left).roundToLong().toInt()
            val minX: Int
            val maxX: Int
            val minY: Int
            val maxY: Int
            if (viewWidth < rect.width()) {
                minX = 0
                maxX = (rect.width() - viewWidth).roundToLong().toInt()
            } else {
                maxX = startX
                minX = maxX
            }
            val startY: Int = (-rect.top).roundToLong().toInt()
            if (viewHeight < rect.height()) {
                minY = 0
                maxY = (rect.height() - viewHeight).roundToLong().toInt()
            } else {
                maxY = startY
                minY = maxY
            }
