...
public class DownloadGallery extends JobIntentService {
    ...
    public static void loadDownloads(Context context) {
        try {
            ...
            for (GalleryDownloaderManager gg : g) {
                gg.downloader().setStatus(GalleryDownloader.Status.PAUSED);
    ...
    public static void startWork(@Nullable Context context) {
        if (context != null)
            enqueueWork(context, DownloadGallery.class, JOB_DOWNLOAD_GALLERY_ID, new Intent());
        synchronized (lock) {
            lock.notify();
        }
    }
    ...
    private void obtainData() {
        GalleryDownloader downloader = DownloadQueue.fetchForData();
